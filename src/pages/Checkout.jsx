...
const itemsToCheckout = isBuyNow ? checkoutItems : cartItems;
  ...
  // 🔥 SHIPPING ESTIMATION (AUTO UPDATE)
useEffect(() => {
  ...
  api.post("/shipping/estimate", {
    ...
    setShippingCharge(res.data.data.charge || 0);
  });
}, [shippingMethod, itemsToCheckout]);
...
// Optional: convert to KG for display
...
if (isBuyNow) {
  orderRes = await api.post("/orders/buy-now", {
    address: selectedAddressId,
    shippingMethod,
    productId: itemsToCheckout[0].productId,
    variantLabel: itemsToCheckout[0].variantLabel,
    quantity: itemsToCheckout[0].quantity,
  });
} else {
  orderRes = await api.post("/orders/cart/checkout", {
    address: selectedAddressId,
    ...
    items: itemsToCheckout.map(item => ({
      ...
      setShippingCharge(orderRes.data.data.shipping.charge); 

      console.log("🧾 FINAL CHECKOUT PAYLOAD", {
  shippingMethod,
  address: selectedAddressId,
});
  ...
  // Render the single address card
  const renderSingleAddressCard = () => {
    const addressParts = formatAddress(defaultAddress);
            ...
            <div className="bg-white rounded-xl sm:rounded-2xl shadow-lg p-4 sm:p-5 lg:p-6">
              <div className="flex items-center justify-between mb-3 sm:mb-4">
                <h3 className="text-lg sm:text-xl font-bold text-gray-900 flex items-center gap-2">
                  <FiShoppingBag className="text-orange-500 text-lg sm:text-xl" />
                 Order Items ({itemsToCheckout.length})
                </h3>
                {activeStep === 1 && (
                ...
                <div className="space-y-3">
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-gray-600">
                        Home delivery via courier (3–5 days)
                      </p>
                    </div>
                    {shippingMethod === "COURIER" && (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-gray-600">
                        Bulk or intercity transport delivery
                      </p>
                    </div>
                    {shippingMethod === "TRANSPORT" && (
                      <FiCheck className="text-green-600 text-xl" />
            ...
            {activeStep >= 4 && (
              <div className="bg-white rounded-xl sm:rounded-2xl shadow-lg p-4 sm:p-6">
                ...
                <div className="mt-6 flex gap-3">
                  ...
                  <button
                    onClick={handlePlaceOrder}
                    disabled={!shippingMethod || isPlacingOrder}
      ...
      {isMobile && activeStep === 4 && (
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t shadow-2xl p-4 z-50">
          <div className="flex items-center justify-between mb-2">
            <div>
              <div className="text-sm text-gray-500">Total Amount</div>
              <div className="text-xl font-bold text-gray-900">₹{totalAmount + shippingCharge}</div>
            </div>
            <button
              onClick={handlePlaceOrder}
              disabled={isPlacingOrder}
