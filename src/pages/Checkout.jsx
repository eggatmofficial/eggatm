...
// Checkout.js - Complete Fixed Version
import { useDispatch, useSelector } from "react-redux";
import { Navigate, useNavigate } from "react-router-dom";
import { useState, useEffect, useCallback } from "react";
import { initiate, verify } from "../api/order.api";
...
import api from "../api/axios";
import { getCartAPI } from "../api/cart.api";
import { setCartFromBackend, clearCart } from "../store/cart.store";
  ...
  const refreshCartAfterPayment = async () => {
    try {
      ...
      setTimeout(async () => {
        try {
          const cartRes = await getCartAPI();
          console.log("🛒 Fresh cart data from backend:", cartRes.data.data);
          dispatch(setCartFromBackend(cartRes.data.data));
        } catch (error) {
          console.error("Failed to fetch cart from backend:", error);
          // Keep cart cleared if backend fails
        }
      }, 500);
