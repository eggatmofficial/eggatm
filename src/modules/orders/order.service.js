...
//before check cart status(06/02/26)
...
//final cart clear  after payment (06/02/26)
...
async updateOrderStatus(orderId, status) {
  ...
  if (order.status !== "PAID" && status === "PAID") {
    ...
    // ✅ REMOVE ITEMS FROM CART ONLY AFTER SUCCESSFUL PAYMENT
    if (order.source === "CART"  || order.source === "BUY_NOW") {
      console.log("🛒 REMOVING ITEMS FROM CART FOR ORDER:", orderId);
      console.log("📋 ORDER ITEMS TO REMOVE:", order.items);
      
      try {
        // Ensure we pass the items in correct format
        const itemsToRemove = order.items.map(item => ({
          productId: item.productId, // This should be ObjectId
          variantLabel: item.variantLabel,
          quantity: item.quantity
        }));
        
        await cartRepo.removeItems(order.userId, itemsToRemove);
        console.log("✅ Cart items removal initiated");
      } catch (cartError) {
        console.error("❌ Error removing cart items:", cartError.message);
        console.error("Full error:", cartError);
        // Don't fail the payment if cart removal fails
      }
    }
  }
